...
2. Sign the payload. ( .env file of the /testing directory should be set )
```bash
cd testing && echo '{"user": "0x9F77cBDb561aaD32b403695306e3eea53F9B40e7", "merchant": "0x9F77cBDb561aaD32b403695306e3eea53F9B40e7", "amount": 18, "nonce": 1}' | node sig-gen-minimal.js
```

3. Simulate CRE triggered authorize function call. User signed signature get relayed by the Merchant. ( Replace the signature )
```bash
cre workflow simulate ./aegis-workflow --http-payload '{"functionName": "authorize", "user": "0x9F77cBDb561aaD32b403695306e3eea53F9B40e7", "merchant": "0x9F77cBDb561aaD32b403695306e3eea53F9B40e7", "merchantType": "RIDE_SHARE", "amount": 18, "nonce": 1, "signature": "0x2b3c4d..."}' --target local-simulation --non-interactive --trigger-index 0
```
...
```bash
make capture-funds PROTOCOL=0xProtocolAddress USER_ADDRESS=0x9F77cBDb561aaD32b403695306e3eea53F9B40e7 AMOUNT=260 FLAGS=--broadcast
```
